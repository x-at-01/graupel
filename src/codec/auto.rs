...
use crate::codec::{Chimp, Chimp128, Codec, Decimal, Elf, FastAlp, Gorilla};
...
impl Codec for Auto {
    ...
    fn encode(&self, points: &[Point]) -> Result<Vec<u8>> {
        ...
        for codec in [&Decimal as &dyn Codec, &Chimp, &Chimp128, &Elf, &FastAlp] {
...
mod tests {
    use super::*;
    use crate::codec::{decode as decode_block, TAG_DECIMAL, TAG_FASTALP};
    ...
    fn picks_decimal_for_tenths() {
        ...
        assert!(block[0] == TAG_DECIMAL || block[0] == TAG_FASTALP);
